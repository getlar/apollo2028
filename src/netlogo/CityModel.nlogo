...
to move-turtle
  let target one-of patches with-max [population-density]
  ifelse random-float 100 < move-tendency [
...
to update-population-density
  ask patches [
    set population-density count turtles-here
    recolor-patch
  ]
end

to recolor-patch
  set pcolor scale-color red population-density 0 max-pop
end
@#$#@#$#@
GRAPHICS-WINDOW
211
...
PENS
"population" 1.0 0 -10899396 true "" "plot count turtles with [color = green]"
